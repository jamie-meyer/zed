    ...
    fn rebuild_contents(&mut self, cx: &mut Context<Self>) {
        ...
        for group in &groups {
            ...
            if self.thread_grouping == ThreadGroupingMode::Worktree {
                ...
                let mut threads_by_worktree: HashMap<PathBuf, Vec<Arc<ThreadEntry>>> =
                    HashMap::new();
                let mut stale_threads_by_worktree: HashMap<PathBuf, Vec<Arc<ThreadEntry>>> =
                    HashMap::new();
                ...
                let mut grouped_worktrees: Vec<(WorktreeListEntry, Vec<Arc<ThreadEntry>>)> =
                    Vec::new();
                ...
                if !self.show_open_worktrees_only {
                    let mut stale_groups =
                        stale_threads_by_worktree.into_iter().collect::<Vec<_>>();
                ...
                let has_thread_notifications =
                    grouped_worktrees.iter().any(|(_, worktree_threads)| {
                        worktree_threads
                            .iter()
                ...
                for (worktree, worktree_threads) in grouped_worktrees {
                    ...
                    if worktree_has_threads {
                        for mut thread in worktree_threads {
                            ...
                            Arc::make_mut(&mut thread).worktrees.clear();
