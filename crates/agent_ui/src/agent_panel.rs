...
use crate::{
    ...
    LoadThreadFromClipboard, NewCodexTerminal, NewTerminalThread, NewThread,
...
fn is_known_terminal_agent_command(command: &str) -> bool {
    canonical_terminal_agent_command(command).is_some()
}

fn canonical_terminal_agent_command(command: &str) -> Option<&'static str> {
    KNOWN_TERMINAL_AGENT_COMMANDS
        .iter()
        .copied()
        .find(|known| command == *known)
        .or_else(|| command.starts_with("codex-").then_some("codex"))
}
...
fn terminal_program_to_report(
    ...
    let current_program = current_program
        .and_then(|program| canonical_terminal_agent_command(&program).map(ToString::to_string));
    ...
    cx.observe_new(
        |workspace: &mut Workspace, _window, _cx: &mut Context<Workspace>| {
            workspace
                ...
                .register_action(|workspace, _: &NewCodexTerminal, window, cx| {
                    if let Some(panel) = workspace.panel::<AgentPanel>(cx) {
                        panel.update(cx, |panel, cx| {
                            panel.new_codex_terminal(
                                Some(workspace),
...
impl AgentTerminal {
    ...
    fn terminal_title_for_persistence(&self, cx: &App) -> SharedString {
        terminal_title_for_persistence(self.terminal_title(cx).as_ref())
    }

    fn title_for_persistence(&self, cx: &App) -> SharedString {
        let terminal_title = self.terminal_title_for_persistence(cx);
        let custom_title = self.custom_title(cx);
        compose_terminal_thread_title(
            terminal_title.as_ref(),
            custom_title.as_ref().map(|title| title.as_ref()),
    ...
    fn refresh_title(&mut self, cx: &mut App) -> bool {
        ...
        let title = self.title_for_persistence(cx);
...
fn terminal_agent_status(terminal: &AgentTerminal, cx: &App) -> Option<AgentThreadStatus> {
    let terminal_entity = terminal.view.read(cx).terminal().clone();
    let terminal = terminal_entity.read(cx);
    terminal_status_from_process_and_title(
        terminal.foreground_process_command_name().as_deref(),
        &terminal.breadcrumb_text,
    )
}

pub(crate) fn terminal_status_from_process_and_title(
    foreground_program: Option<&str>,
    terminal_title: &str,
) -> Option<AgentThreadStatus> {
    let foreground_program = canonical_terminal_agent_command(foreground_program?)?;

    if foreground_program != "codex" {
        return Some(AgentThreadStatus::Running);
    }

    Some(codex_status_from_title(terminal_title))
}

pub(crate) fn codex_status_from_title(terminal_title: &str) -> AgentThreadStatus {
    if terminal_title.starts_with("[ ! ] Action Required")
        || terminal_title.starts_with("[ . ] Action Required")
    {
        return AgentThreadStatus::WaitingForConfirmation;
    }

    const CODEX_SPINNER_FRAMES: [char; 10] = ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏'];
    if terminal_title
        .chars()
        .next()
        .is_some_and(|character| CODEX_SPINNER_FRAMES.contains(&character))
        || terminal_title
            .split(" | ")
            .any(|part| matches!(part, "Starting" | "Working" | "Waiting" | "Thinking"))
        || terminal_title.is_empty()
    {
...
pub struct AgentPanel {
    ...
    _terminal_status_store_subscription: Subscription,
    last_context_source: Option<AgentContextSource>,
    replace_restored_thread_with_codex_on_activation: bool,
...
impl AgentPanel {
        ...
        cx.spawn(async move |cx| {
            ...
            let terminal_id_to_restore = if has_open_project {
                serialized_panel
                    ...
                    .and_then(
                        |terminal_id| match TerminalId::from_key_string(terminal_id) {
                    ...
                    match cx.update(|_window, cx| {
                        ...
                        Ok(Some((store, reload_task))) => {
                            ...
                            let thread_id = store.read_with(cx, |store, _cx| {
                                let primary = info.thread_id.and_then(|thread_id| store.entry(thread_id));
                                let fallback = info.session_id.as_ref().and_then(|session_id| {
                                    store.entry_by_session(&acp::SessionId::new(session_id.clone()))
                                });
                                primary
                                    .or(fallback)
                        ...
                        Err(error) => {
                            log::warn!("failed to access thread metadata store: {error}");
                            None
                        }
                    }
                } else {
            ...
            let panel = workspace.update_in(cx, |workspace, window, cx| {
                ...
                panel.update(cx, |panel, cx| {
                    ...
                    // Ignore an old ACP preference so upgrading cannot reopen the retired workflow.
                    ...
                    if let Some(metadata) = terminal_to_restore {
                        ...
                        panel.replace_restored_thread_with_codex_on_activation = true;
                    }
                    if let Some(new_draft_thread_id) = serialized_panel
                        .as_ref()
                        .and_then(|panel| panel.new_draft_thread_id)
                    {
    ...
    pub(crate) fn new(workspace: &Workspace, window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let _terminal_status_store_subscription = cx.subscribe_in(
            &terminal_status_store,
            window,
            |_this, _store, event: &TerminalThreadStatusChanged, window, cx| {
                let was_active = matches!(
                    event.previous_status,
                    Some(AgentThreadStatus::Running | AgentThreadStatus::WaitingForConfirmation)
                );
                if was_active && event.status == AgentThreadStatus::Completed {
        ...
        let panel = Self {
            workspace_id,
            base_view,
            last_created_entry_kind: AgentPanelEntryKind::Terminal,
            overlay_view: None,
            workspace,
            user_store,
            ...
            _terminal_status_store_subscription,
            last_context_source: None,
            replace_restored_thread_with_codex_on_activation: false,
            is_active: false,
        };
    ...
    fn restore_new_draft(
        ...
        let Some(metadata) = ThreadMetadataStore::try_global(cx)
            ...
            .filter(|metadata| metadata.is_draft())
        else {
            return;
        };
    ...
    pub fn ensure_codex_terminal(
        ...
        if self.terminals.is_empty() && self.pending_terminal_spawn.is_none() {
            self.new_codex_terminal(workspace, source, window, cx);
        }
    }
    ...
    fn spawn_terminal(
        ...
        let ensure_session_task = if self.project.read(cx).is_local() {
            ...
            Task::ready(Err(anyhow!(
                "Persistent Terminal Threads require tmux on the project host; remote projects are not supported yet"
            )))
        };
        ...
        cx.spawn_in(window, async move |this, cx| {
            let session_created = match ensure_session_task.await {
                Ok(session_created) => session_created,
                Err(error) => {
                    log::error!("failed to create tmux terminal session: {error:#}");
                    workspace
            ...
            #[cfg(not(any(test, feature = "test-support")))]
            let attach_shell = tmux_session::attach_shell(terminal_id);
            #[cfg(not(any(test, feature = "test-support")))]
            let (program, args) = attach_shell.program_and_args();
            #[cfg(not(any(test, feature = "test-support")))]
            let terminal_task = project.update(cx, |project, cx| {
                project.create_terminal_shell_with_program(
                    working_directory,
                    program,
                    args.to_vec(),
                    cx,
            ...
            let terminal = match terminal_task.await {
                ...
                Err(error) => {
                    ...
                    if session_created {
                        tmux_session::kill_session(terminal_id).await.log_err();
                    }
                    workspace
                        .update(cx, |workspace, cx| workspace.show_error(error, cx))
                        .log_err();
    ...
    pub fn close_terminal(
        ...
        self.kill_and_close_terminal(terminal_id, true, window, cx);
    }
    ...
    pub fn close_terminal_without_activating_draft(
        ...
        self.kill_and_close_terminal(terminal_id, false, window, cx);
    }

    pub fn close_terminal_after_tmux_killed(
        &mut self,
        terminal_id: TerminalId,
    ...
    fn kill_and_close_terminal(
        ...
        #[cfg(not(any(test, feature = "test-support")))]
        {
            let workspace = self.workspace.clone();
            cx.spawn_in(window, async move |this, cx| {
                if let Err(error) = tmux_session::kill_session(terminal_id).await {
                    workspace
                        .update(cx, |workspace, cx| workspace.show_error(error, cx))
                        .log_err();
                    return anyhow::Ok(());
    ...
    fn refresh_terminal_status(&self, terminal_id: TerminalId, cx: &mut App) {
        let Some(status) = self
            .terminals
            .get(&terminal_id)
            .and_then(|terminal| terminal_agent_status(terminal, cx))
        else {
            return;
        };
        TerminalThreadStatusStore::global(cx).update(cx, |store, cx| {
    ...
    pub fn restore_terminal(
        ...
        let initial_command = metadata.initial_command.clone().or_else(|| {
            metadata
                .custom_title
                .as_ref()
    ...
    fn pop_up_terminal_notification(
        ...
        let agent_panel_subscription = cx.subscribe_in(&this_panel, window, {
            move |this, _, event: &AgentPanelEvent, window, cx| match event {
                ...
                AgentPanelEvent::EntryChanged | AgentPanelEvent::ThreadInteracted { .. } => {}
            }
        });
...
impl Panel for AgentPanel {
    ...
    fn set_active(&mut self, active: bool, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if active {
            if self.replace_restored_thread_with_codex_on_activation {
                self.replace_restored_thread_with_codex_on_activation = false;
                cx.defer_in(window, |this, window, cx| {
                    if matches!(this.base_view, BaseView::AgentThread { .. })
                        && this.supports_terminal(cx)
                    {
...
impl AgentPanel {
    ...
    fn render_toolbar(&self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let showing_terminal = matches!(self.visible_surface(), VisibleSurface::Terminal(_));
        let worktree_sidebar_open = window
            .root::<MultiWorkspace>()
            .flatten()
            .is_some_and(|multi_workspace| multi_workspace.read(cx).worktree_sidebar_open(cx));
            ...
            Rc::new(move |window, cx| {
                Some(ContextMenu::build(window, cx, |menu, _window, _cx| {
                    menu.context(focus_handle.clone())
                        .item(
                            ContextMenuEntry::new("Codex Terminal")
                                .action(Box::new(NewCodexTerminal))
        ...
        let toolbar_content = {
            ...
            let worktree_view_button = IconButton::new("worktree-view", IconName::GitWorktree)
                .icon_size(IconSize::Small)
                .toggle_state(worktree_sidebar_open)
                .tooltip(Tooltip::text("Toggle Worktree View"))
            ...
            base_container
                ...
                .child(
                    h_flex()
                        ...
                        .when(can_create_entries, |this| this.child(worktree_view_button))
...
mod tests {
    ...
    fn test_is_known_terminal_agent_command() {
        ...
        assert!(is_known_terminal_agent_command("codex-aarch64-a"));
    ...
    fn test_terminal_program_reports_known_agent_transitions() {
        ...
        assert_eq!(
            terminal_program_to_report(
                ...
                Some("codex-aarch64-a".to_string())
            ),
            None
        );
        assert_eq!(
    ...
    #[test]
    fn test_codex_terminal_status_follows_terminal_title_protocol() {
        assert_eq!(
            terminal_status_from_process_and_title(Some("codex"), "[ ! ] Action Required | zed"),
        ...
        assert_eq!(
            terminal_status_from_process_and_title(Some("codex"), "⠋ zed"),
            Some(AgentThreadStatus::Running)
        );
        assert_eq!(
            terminal_status_from_process_and_title(Some("codex"), "zed | Thinking"),
            Some(AgentThreadStatus::Running)
        );
        assert_eq!(
            terminal_status_from_process_and_title(Some("codex"), "zed"),
        ...
        assert_eq!(
            terminal_status_from_process_and_title(
                Some("codex-aarch64-a"),
                "[ ! ] Action Required | zed"
            ),
            Some(AgentThreadStatus::WaitingForConfirmation)
        );
        assert_eq!(
            terminal_status_from_process_and_title(Some("cargo"), "⠋ zed"),
            None
        );
    }
    ...
    #[gpui::test]
    async fn test_terminal_client_close_retains_metadata_without_sidebar(cx: &mut TestAppContext) {
        let (panel, mut cx) = setup_panel(cx).await;
        cx.update(|_, cx| {
            TerminalThreadMetadataStore::init_global(cx);
        });
        ...
        cx.update(|_, cx| {
            assert!(
                TerminalThreadMetadataStore::global(cx)
                    ...
                    .entry(terminal_id)
                    .is_some(),
                "detaching the terminal client must retain persistent tmux metadata"
            );
        });
    }
    ...
    #[gpui::test]
    async fn test_terminal_completion_status_marks_notification(cx: &mut TestAppContext) {
        let (panel, mut cx) = setup_panel(cx).await;
        let first_terminal_id = panel
            .update_in(&mut cx, |panel, window, cx| {
                panel.insert_test_terminal("Build", true, window, cx)
        ...
        cx.update(|_window, cx| {
            TerminalThreadStatusStore::global(cx).update(cx, |store, cx| {
                store.set_status(first_terminal_id, AgentThreadStatus::Running, cx);
                store.set_status(first_terminal_id, AgentThreadStatus::Completed, cx);
            });
        });
        cx.run_until_parked();
